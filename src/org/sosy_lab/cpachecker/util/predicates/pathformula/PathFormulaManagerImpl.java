...
public class PathFormulaManagerImpl implements PathFormulaManager {
	...
	@Option(secure = true, name = "simplifyGeneratedPathFormulas", description = "Call 'simplify' on generated formulas.")
	...
	private PathFormulaBuilderVariants pathFormulaBuilderVariant =
			PathFormulaBuilderVariants.DEFAULT;
	...
	public PathFormulaManagerImpl(
		...
		if (handlePointerAliasing) { // False
	...
	private PathFormula
		...
		if (useNondetFlags) {
			...
			if (lNondetIndex != lFlagIndex) {
				if (lFlagIndex < 0) {
					lFlagIndex = 1; // ssa indices start with 2, so next flag that is generated also
									// uses index 2
				}
				...
				for (int lIndex = lFlagIndex + 1; lIndex <= lNondetIndex; lIndex++) {
					Formula nondetVar =
							fmgr.makeVariable(NONDET_FORMULA_TYPE, NONDET_FLAG_VARIABLE, lIndex);
				...
				// setSsaIndex(ssa, Variable.create(NONDET_FLAG_VARIABLE, getNondetType()),
				// lNondetIndex);
	...
	public PathFormula makeOr(final PathFormula pathFormula1, final PathFormula pathFormula2)
		...
		final BooleanFormula newFormula1 =
				bfmgr.and(
						formula1,
						bfmgr.and(
								mergeSSAResult.getLeftConjunct(),
								mergePtsResult.getLeftConjunct()));
		final BooleanFormula newFormula2 =
				bfmgr.and(
						formula2,
						bfmgr.and(
								mergeSSAResult.getRightConjunct(),
								mergePtsResult.getRightConjunct()));
		final BooleanFormula newFormula =
				bfmgr.and(
						bfmgr.or(newFormula1, newFormula2),
						bfmgr.and(
								mergeSSAResult.getFinalConjunct(),
								mergePtsResult.getFinalConjunct()));
	...
	/**
	 * Build a formula containing a predicate for all branching situations in the ARG. If a
	 * satisfying assignment is created for this formula, it can be used to find out which paths in
	 * the ARG are feasible.
	 *
			...
			if (childrenOnPath.size() > 1) {
				if (childrenOnPath.size() > 2) {
					...
					if (from(childrenOnPath).anyMatch(AbstractStates::isTargetState)) {
						// We expect this situation of one of the children is a target state created
						// by PredicateCPA.
						continue;
					} else {
						logger.log(
				...
				if (!outgoingEdges.allMatch(Predicates.instanceOf(AssumeEdge.class))) {
					if (from(childrenOnPath).anyMatch(AbstractStates::isTargetState)) {
						...
						logger.log(
								Level.WARNING,
								"ARG branching without AssumeEdge at ARG node "
										+ pathElement.getStateId()
										+ ".");
						return bfmgr.makeTrue();
					}
				...
				if (positiveEdge == null || negativeEdge == null) {
					logger.log(
							Level.WARNING,
							"Ambiguous ARG branching at ARG node "
									+ pathElement.getStateId()
									+ ".");
					return bfmgr.makeTrue();
				}
				...
				if (pf == null) {
					...
					// TODO the class PathFormulaManagerImpl should not depend on
					// PredicateAbstractState,
					// it is used without PredicateCPA as well.
					PredicateAbstractState pe =
							AbstractStates
									.extractStateByType(pathElement, PredicateAbstractState.class);
					if (pe == null) {
						logger.log(
								Level.WARNING,
	 ...
	 *
	 * <p>
	 * A map is created that stores for each ARGState (using its element id as the map key) which
	...
	public ImmutableMap<Integer, Boolean>
		...
		for (ValueAssignment entry : model) {
			...
			if (entry.getKey() instanceof BooleanFormula) {
				String name =
						BRANCHING_PREDICATE_NAME_PATTERN.matcher(canonicalName).replaceFirst("");
