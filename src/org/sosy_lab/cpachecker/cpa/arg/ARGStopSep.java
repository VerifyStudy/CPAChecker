...
public class ARGStopSep implements StopOperator, ForcedCoveringStopOperator {
  ...
  private boolean stop(ARGState pElement, ARGState pReachedState, Precision pPrecision)
    ...
    if (Objects.equals(pElement, pReachedState)) {
      logger.log(Level.INFO, "cover failed:", pReachedState.getStateId(), "==", pElement.getStateId());
      return false;
    }
    ...
    if (stop) {
      logger.log(Level.INFO, "cover success:", pReachedState.getStateId(), "->", pElement.getStateId(),
        "\n", PredicateAbstractState.getPredicateState(pReachedState).getAbstractionFormula().asFormula().toString(),
        "\n=>", PredicateAbstractState.getPredicateState(pElement).getAbstractionFormula().asFormula().toString());
