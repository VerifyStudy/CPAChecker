...
public class ARGReachedSet {
  ...
  public ARGReachedSet(ReachedSet pReached, ConfigurableProgramAnalysis pCpa, LogManager pLogger) {
    this(pReached, pCpa, -1);
    this.logger = pLogger;
  }
  /**
  ...
  public void removeCoverageOf(ARGState v) {
    for (ARGState coveredByChildOfV : ImmutableList.copyOf(v.getCoveredByThis())) {
      logger.log(Level.INFO, "cleaning cover:", v.getStateId(), "->", coveredByChildOfV.getStateId());
  ...
  private void uncover(ARGState element) {
    ...
    for (ARGState e : element.getSubgraph()) {
      ...
      if (!e.wasExpanded()) {
        ...
        logger.log(Level.INFO, "Id", e.getStateId(), "reAddToWaitlist");
      }
    }
  }
  ...
  public boolean tryToCover(ARGState v) throws CPAException, InterruptedException {
    ...
    Collection<AbstractState> coverReachedSet = mReached.getReached(v);
    logger.log(Level.INFO, "try to find", coverReachedSet.size(), "cover to id:", v.getStateId());
    // sideeffect: coverage and cleanup of ARG is done in ARGStopSep#stop
    boolean stop = cpa.getStopOperator().stop(v, coverReachedSet, mReached.getPrecision(v)); // ARGStopSep
    ...
    if (v.isCovered()) {
      ...
      logger.log(Level.INFO, "clearning state subtree(cover to)", v.getStateId());
      removeCoverageOf(v);
      ...
      logger.log(Level.INFO, "clearning state subtree(cover by)", v.getStateId());
      for (ARGState childOfV : subtree) {
