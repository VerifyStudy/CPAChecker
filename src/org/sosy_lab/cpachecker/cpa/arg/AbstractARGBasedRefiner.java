...
public class AbstractARGBasedRefiner implements Refiner, StatisticsProvider {
	...
	public static Refiner create(ConfigurableProgramAnalysis pCpa)
		...
		return AbstractARGBasedRefiner
				.forARGBasedRefiner((rs, path) -> CounterexampleInfo.feasibleImprecise(path), pCpa);
	}
	...
	public final boolean performRefinement(ReachedSet pReached)
		...
		logger.log(Level.INFO, "Starting ARG based refinement");
		...
		logger.log(
				Level.FINEST,
				"ARG based refinement finished, result is",
				counterexample.isSpurious());
	...
	protected CounterexampleInfo performRefinementForPath(ARGReachedSet pReached, ARGPath pPath)
		...
		return refiner.performRefinementForPath(pReached, pPath); // refiner := PredicateCPARefiner
	}

	/**
	 * This method may be overwritten if the standard behavior of
	 * <code>ARGUtils.getOnePathTo()</code> is not appropriate in the implementations context.
	 *
