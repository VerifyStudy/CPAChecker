...
public class PredicateBasedPrefixProvider implements PrefixProvider {
	...
	@Option(
		secure = true,
		description = "Max. length of feasible prefixes to extract from if at least one prefix was already"
				+ " extracted")
	...
	public static PredicateBasedPrefixProvider create(ConfigurableProgramAnalysis pCpa)
		...
		PredicateCPA predicateCpa =
				CPAs.retrieveCPAOrFail(
						pCpa,
						PredicateCPA.class,
						PredicateBasedPrefixProvider.class);
	...
	public List<InfeasiblePrefix> extractInfeasiblePrefixes(final ARGPath pPath)
		...
		try (InterpolatingProverEnvironment<?> prover =
			...
			// 从路径公式中提取一组切片
			logger.log(Level.INFO, "Path", pPath);
			logger.log(Level.INFO, "block formulas", blockFormulas);
		...
		for (RawInfeasiblePrefix rawPrefix : rawPrefixes) {
			infeasiblePrefixes.add(
					InfeasiblePrefix
							.buildForPredicateDomain(rawPrefix, solver.getFormulaManager()));
		}
		...
		while (iterator.hasNext()) {
			...
			if (isAbstractionState(currentState)) {
				...
				try {
					...
					terms.add(term);
					Boolean flag = checkUnsat(pPath, iterator.getOutgoingEdge());
					logger.log(Level.INFO, flag, currentBlockFormula);

					if (flag && prover.isUnsat()) {
						...
						logger.log(Level.INFO, "interpolantSequence:", interpolantSequence);
						...
						// failing assume operations replaced with no-ops, as this is not needed
						// here,
					...
					throw new CPAException(
							"Error during computation of prefixes: " + e.getMessage(),
							e);
				}
	...
	private <T> List<BooleanFormula> extractInterpolantSequence(
			final List<T> pTerms,
			final InterpolatingProverEnvironment<T> pProver)
	 ...
	 * then unsatisfiability has to be checked always. In case it is single-block encoded, then it
	 * suffices to check unsatisfiability at assume edges.
	 *
	...
	/**
	 * This method checks if the given path is single-block-encoded, which is the case, if all
	 * states in the path are abstraction states.
	 *
	 * @param pPath the path to check
	...
	private boolean isSingleBlockEncoded(final ARGPath pPath) {
		return from(pPath.asStatesList())
				.allMatch(PredicateAbstractState::containsAbstractionState);
	}
