...
public class PredicateAbstractDomain implements AbstractDomain {
  ...
  public boolean isLessOrEqual(AbstractState element1, AbstractState element2)
    ...
    try {
       ...
       * Math.max(stats.bddCoverageCheckMaxTime, (end - start)); stats.bddCoverageCheckTime += (end
       * - start); ++stats.numCoverageChecks;
       */
      ...
      if (e1.isAbstractionState() && e2.isAbstractionState()) {
        ...
        if (result) {
          logger.log(
              Level.INFO,
              "cover success:",
              e1.getAbstractionFormula().asFormula().toString(),
              "\n=>",
              e2.getAbstractionFormula().asFormula().toString());
        }
