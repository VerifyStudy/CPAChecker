...
public class ImpactGlobalRefiner implements Refiner, StatisticsProvider {
	...
	private void printStatistics(PrintStream out) {
		if (refinementCalls > 0) {
			...
			out.println(
					"Avg. number of refined paths per iteration: "
							+ div(pathsRefined, refinementIterations));
			out.println(
	...
	public boolean performRefinement(ReachedSet pReached)
		...
		try {
			...
			List<AbstractState> targets =
					from(pReached).filter(AbstractStates::isTargetState).toList();
	 ...
	 * on the paths to the target states, and then call
	...
	private boolean performRefinement0(ReachedSet pReached, List<AbstractState> targets)
		...
		logger.log(Level.INFO, "Starting refinement for", targets.size(), "elements.");
		...
		for(var K : successors.keySet()){
			sBuffer.append("[ ");
			sBuffer.append(K.getStateId());
			sBuffer.append(" -> ");
			for(var V : successors.get(K)){
				sBuffer.append(V.getStateId() + " ");
			}
			sBuffer.append("] ");
		}
		String pairs = sBuffer.toString();
		logger.log(Level.INFO, "CEP:", pairs);
	 ...
	 * recursion step corresponds to one "block" of the ARG. As one block may have several
	 * successors, this is recursion on a tree. We proceed in a DFS order. Recursion stops as soon
	 * as the path has been determined to be infeasible (so we do refinement as soon as possible) or
	 * a target state is reached (then we found a feasible counterexample). When an infeasible state
	 * was found, we call
	 * {@link #performRefinementOnPath(List, ARGState, Map, ReachedSet, InterpolatingProverEnvironment)}
	 * to do the actual refinement.
	 *
	 * Note that the successor and predecessor relation contains only states that belong to paths to
	 * a target state, so we refine only such paths, and not all paths in the ARG.
	 *
	...
	private <T> boolean step(
		...
		for (ARGState succ : successors.get(current)) {
			...
			logger.log(Level.INFO, "BooleanFormula", succ.getStateId(), blockFormula);
			...
			try {
				...
				satCheckTime.stop();
				if (isUnsat) {
					logger.log(Level.INFO, "Found unreachable state", succ.getStateId());
					performRefinementOnPath(
							unmodifiableList(itpStack),
							succ,
							predecessors,
							pReached,
							itpProver);
					// logger.log(Level.INFO, blockFormula);
				} else if (targets.contains(succ)) {
					// We have found a reachable target state, immediately abort refinement.
					logger.log(Level.INFO, "Found reachable target state", succ.getStateId());
					return false;
				...
				if (!current.mayCover()) {
					// The refinement along the current path made the current part of the ARG
					// covered.
	 ...
	 * {@link #performRefinementForState(BooleanFormula, ARGState)} once for each interpolant and
	 * its corresponding state. Afterwards we call
	 * {@link #finishRefinementOfPath(ARGState, List, ReachedSet)} once.
	 *
	...
	private <T> void performRefinementOnPath(
		...
		do {
			...
			logger.log(Level.INFO, "itp on", currentState.getStateId(), currentItp);
			getInterpolantTime.stop();

			if (bfmgr.isTrue(currentItp)) {
				// from here to the ARG root, all interpolants will be True
				logger.log(Level.INFO, "itp: True \nhere to the ARG root, all interpolants will be True");
				break;
			}
		...
		affectedStates = Lists.reverse(affectedStates); // reverse so that they are in top-down
														// order
	 ...
	 * this state. This method is only called for states for which there is a non-trivial
	 * interpolant (i.e., neither True nor False).
	 *
	 ...
	 * @return True if no refinement was necessary (this implies that refinement on all of the
	 *         state's parents is also not necessary)
	 */
	private boolean performRefinementForState(BooleanFormula interpolant, ARGState state)
		...
		String beforeStr = new String(getPredicateState(state).getAbstractionFormula().asFormula().toString());
		
		boolean stateChanged = impact.strengthenStateWithInterpolant(interpolant, state, null);

		String afterStr = new String(getPredicateState(state).getAbstractionFormula().asFormula().toString());
		if (stateChanged)
			logger.log(Level.INFO, "(itp => state) = unsat\nstrength:", stateChanged, "\n", beforeStr, "\n=>", afterStr);
		else
			logger.log(Level.INFO, "(itp => state) = sat\nstrength:", stateChanged, "\n", beforeStr, "\n!=>", afterStr);
	 ...
	 * the infeasible part from the ARG, and re-establish the coverage invariant (i.e., that states
	 * on the path are either covered or cannot be covered).
	 *
	 * @param unreachableState The first state in the path which is infeasible (this identifies the
	 *                         path).
	...
	private void finishRefinementOfPath(
			...
			ReachedSet reached)
			throws CPAException, InterruptedException {
		ARGReachedSet arg = new ARGReachedSet(reached, argCpa, logger);

		argUpdate.start();
		for (ARGState w : affectedStates) {
			logger.log(Level.INFO, "trying to clear cover:", w.getStateId(), "-> * ");
			arg.removeCoverageOf(w);
		}

		// remove ARG part from unreachableState downwards
		Set<ARGState> infeasibleSubtree = unreachableState.getSubgraph().toSet();
		List<Integer> idSet = new ArrayList<>();
		for(var subSrate : infeasibleSubtree){
			idSet.add(subSrate.getStateId());
		}
		logger.log(Level.INFO, "remove", unreachableState.getStateId(), " sub tree", idSet,"from ART");
