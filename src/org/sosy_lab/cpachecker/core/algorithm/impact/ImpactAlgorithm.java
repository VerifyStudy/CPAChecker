...
public class ImpactAlgorithm implements Algorithm, StatisticsProvider {
	...
	private class Stats implements Statistics {
		...
		@Override
		public void
				printStatistics(PrintStream out, Result pResult, UnmodifiableReachedSet pReached) {
			...
			out.println(
					"Number of refinements:              " + refinementTime.getNumberOfIntervals());
			if (useForcedCovering) {
				out.println(
						"Number of forced coverings:         "
								+ forceCoverTime.getNumberOfIntervals());
			...
			for (CFAEdge edge : leavingEdges(loc)) {
				...
				Collection<? extends AbstractState> successors =
						cpa.getTransferRelation()
								.getAbstractSuccessorsForEdge(predecessor, precision, edge);
				...
				Vertex w =
						new Vertex(
								...
								v,
								bfmgr.makeTrue(),
								Iterables.getOnlyElement(successors));
	...
	/**
	 * Check if a vertex v may potentially be covered by another vertex w. It checks everything
	 * except their state formulas.
	 */
	private boolean mayCover(Vertex v, Vertex w, Precision prec)
		...
		return (v != w)
				...
				&& cpa.getStopOperator()
						.stop(
								v.getWrappedState(),
								Collections.singleton(w.getWrappedState()),
								prec);
	}
	...
	private boolean forceCover(Vertex v, Vertex w, Precision prec)
		...
		List<BooleanFormula> formulas = new ArrayList<>(path.size() + 2);
		{
			...
			formulas.add(
					fmgr.instantiate(x.getStateFormula(), SSAMap.emptySSAMap().withDefault(1)));
			...
			formulas.add(
					bfmgr.not(fmgr.instantiate(w.getStateFormula(), pf.getSsa().withDefault(1))));
		}
	...
	private boolean close(Vertex v, ReachedSet reached)
		...
		try {
			...
			Collection<AbstractState> sameLocatioStates = reached.getReached(v);
			for (AbstractState ae : sameLocatioStates) {
				Vertex w = (Vertex) ae;
	...
	private boolean dfs(Vertex pVertex, ReachedSet reached)
		...
		while (!stack.isEmpty()) { // 如果栈stack不为空就一直检测
			Vertex v = stack.pop(); // 出栈一个 v
	...
	// 利用Path管理器把path上的公式添加到pathFormulas
