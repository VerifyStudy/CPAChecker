...
public class RestartAlgorithm extends NestingAlgorithm implements ReachedSetUpdater {
	...
	private static class RestartAlgorithmStatistics extends MultiStatistics {
		...
		private void
				printIntermediateStatistics(PrintStream out, Result result, ReachedSet reached) {

			String text =
					"Statistics for algorithm " + noOfAlgorithmsUsed + " of " + noOfAlgorithms;
			out.println(text);
		...
		private void
				printSubStatistics(PrintStream out, Result result, UnmodifiableReachedSet reached) {
	...
	@Option(
		secure = true,
		required = true,
		description = "List of files with configurations to use. "
	...
	@Option(
		secure = true,
		description = "print the statistics of each component of the restart algorithm"
				+ " directly after the components computation is finished")
	private boolean printIntermediateStatistics = true;

	@Option(
		secure = true,
		description = "let each component of the restart algorithm write output files"
				+ " and not only the last one that is excuted")
	private boolean writeIntermediateOutputFiles = false;

	/*
	 * The option is useful for some preanalysis, for instance, the first analysis is fast and
	 * provides some hints to the next ones Is used, for example, in CPALockator
	 *
	 * TODO It might be better to have two lists of algorithms given to the RestartAlgorithm. One
	 * list for analysis with no result expected except information about the program and a second
	 * list for the real analyses to be restarted if necessary. This would allow to combine the
	 * pre-computation of information and the normal sequential composition of algorithms in a more
	 * flexible way.
	 */
	@Option(
		secure = true,
	...
	public AlgorithmStatus run(ReachedSet pReached) throws CPAException, InterruptedException {
		...
		for (var file : configFiles){
			logger.log(Level.INFO, file.value().toString());
		}
		...
		while (configFilesIterator.hasNext()) {
			...
			try {
				...
				try {
					Triple<Algorithm, ConfigurableProgramAnalysis, ReachedSet> currentAlg =
							createNextAlgorithm(
									...
									// we can only use the reached set if the last analysis
				...
				if (currentAlgorithm instanceof StatisticsProvider) {
					((StatisticsProvider) currentAlgorithm)
				...
				try {
					...
					if (currentReached.hasViolatedProperties() && status.isPrecise()) {
						...
						// If the algorithm is not _precise_, verdict "false" actually means
						// "unknown".
						return status;
					}
					...
					if (configFilesIterator.hasNext()) {
						logger.logUserException(
								Level.WARNING,
								e,
								"Analysis " + stats.noOfAlgorithmsUsed + " not completed.");
				...
				singleShutdownManager.requestShutdown("Analysis terminated"); // shutdown any
																				// remaining
																				// components
