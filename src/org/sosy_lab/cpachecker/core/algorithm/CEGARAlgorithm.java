...
public class CEGARAlgorithm implements Algorithm, StatisticsProvider, ReachedSetUpdater {
	...
	private static class CEGARStatistics implements Statistics {
		...
		public void
			...
			if (countRefinements > 0) {
				...
				out.println(
						"Max. size of reached set before ref.: " + maxReachedSizeBeforeRefinement);
				out.println(
						"Max. size of reached set after ref.:  " + maxReachedSizeAfterRefinement);
				out.println(
	...
	public static class CEGARAlgorithmFactory implements AlgorithmFactory {
		...
		/*
		 * Widely used in CPALockator, as there are many error paths, and refinement all of them
		 * takes too much time, so, limit refinement iterations and remove at least some infeasible
		 * paths
		 */
		@Option(
			name = "maxIterations",
	...
	public AlgorithmStatus run(ReachedSet reached) throws CPAException, InterruptedException {
		...
		try {
			...
			do {
				...
				// run algorithm
				logger.log(Level.INFO, "generating ART starting");
				status = status.update(algorithm.run(reached));
				logger.log(Level.INFO, "generating ART ending");
	...
	private boolean refine(ReachedSet reached) throws CPAException, InterruptedException {
		logger.log(Level.INFO, "Error found, performing CEGAR");
		// logger.log(Level.INFO, mRefiner); // PredicateCPARefiner
		...
		logger.log(Level.INFO, "Refinement successful:", refinementResult);
