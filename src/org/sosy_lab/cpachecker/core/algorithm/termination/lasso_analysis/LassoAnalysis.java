...
public class LassoAnalysis {
    ...
    private String externalSolverCommand =
            NativeLibraries.getNativeLibraryPath().resolve("z3")
    ...
    public static LassoAnalysis create(
        ...
        SolverContext solverContext =
                SolverContextFactory
                        .createSolverContext(pConfig, pLogger, pShutdownNotifier, SMTINTERPOL);
        ...
        LassoBuilder lassoBuilder =
                new LassoBuilder(
                        ...
                        formulaManager,
                        formulaManagerView,
                        () -> solverContext.newProverEnvironment(), // Eclipse compiler crashes if a
                                                                    // method reference is
                                                                    // used here.
                        pathFormulaManager,
                        pStatistics);
    ...
    private LassoAnalysis(
        ...
        linearLassoRankerPreferences =
                new LassoRankerPreferences(new DefaultLassoRankerPreferences() {
        ...
        nonTerminationAnalysisSettings =
                new NonTerminationAnalysisSettings(new DefaultNonTerminationAnalysisSettings() {
        ...
        linearTerminationAnalysisSettings =
                new TerminationAnalysisSettings(new DefaultTerminationAnalysisSettings() {
    ...
    private LassoAnalysisResult checkTermination0(
        ...
        try {
            ...
            logger.logUserException(
                    Level.WARNING,
                    e,
                    "Could not check (non)-termination of lasso (" + pLoop + ").");
            return LassoAnalysisResult.unknown();
    ...
    public LassoAnalysisResult checkTermination(
        ...
        for (Lasso lasso : lassos) {
            ...
            logger.logf(
                    Level.FINER,
                    "Synthesizing non-termination argument for lasso:\n%s.",
                    lasso);
    ...
    private LassoAnalysisResult synthesizeNonTerminationArgument(Loop pLoop, Lasso lasso)
        ...
        try (NonTerminationArgumentSynthesizer nonTerminationArgumentSynthesizer =
            ...
            if (result.equals(LBool.SAT)
                    && nonTerminationArgumentSynthesizer.synthesisSuccessful()) {
            ...
            for (RankingTemplate rankingTemplate : rankingTemplates) {
                ...
                try (TerminationArgumentSynthesizer terminationArgumentSynthesizer =
                    ...
                    try {
                        ...
                        // Workaround for a bug in LassoRanker
                    ...
                    if (result.equals(LBool.SAT)
                            && terminationArgumentSynthesizer.synthesisSuccessful()) {
                        TerminationArgument terminationArgument =
                                terminationArgumentSynthesizer.getArgument();
                        logger.logf(
                                Level.FINE,
                                "Found termination argument: %s",
                                terminationArgument);
                        ...
                        try (ProverEnvironment proverEnv = solverContext.newProverEnvironment()) {
                            ...
                            if (!proverEnv.isUnsat()) {
                                statistics
                                        .synthesizedTerminationArgument(pLoop, terminationArgument);
                            ...
                            logger.logUserException(
                                    Level.INFO,
                                    e,
                                    "Could not create ranking relation from "
                                            + terminationArgument);
                            return LassoAnalysisResult.unknown();
                        }
                    }
    ...
    private TerminationArgumentSynthesizer
            createTerminationArgumentSynthesizer(Lasso lasso, RankingTemplate template)
                    throws IOException {
