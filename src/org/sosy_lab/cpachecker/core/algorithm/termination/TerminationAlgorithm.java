...
public class TerminationAlgorithm implements Algorithm, AutoCloseable, StatisticsProvider {
	...
	@Option(
		secure = true,
		description = "consider counterexamples for loops for which only pointer variables are relevant or which check that pointer is unequal to null pointer to be imprecise")
	...
	public TerminationAlgorithm(
		...
		TerminationCPA terminationCpa =
				CPAs.retrieveCPAOrFail(
						pSafetyCPA,
						TerminationCPA.class,
						TerminationAlgorithm.class);
	...
	private AlgorithmStatus run0(ReachedSet pReachedSet)
			throws InterruptedException, CPAEnabledAnalysisPropertyViolationException,
			CPAException {
		...
		while (pReachedSet.hasWaitingState() && result != Result.FALSE) {
			...
			if (status.isPrecise() && targetStateWithCounterExample.isPresent()) {
				...
				CounterexampleInfo counterexample =
						removeDummyLocationsFromCounterExample(
								originalCounterexample,
								nonTerminationLoopHead);
				...
				if (lassoAnalysisResult.hasNonTerminationArgument()) {
					...
					if (rankingRelations.add(rankingRelation)) {
						...
						logger.logf(
								WARNING,
								"Repeated ranking relation %s for %s",
								rankingRelation,
								pLoop);

						// Do not use the first reached target state again and again
						// if we cannot synthesis new termination arguments from it.
						if (repeatedRankingFunctionsSinceSuccessfulIteration > maxRepeatedRankingFunctionsPerLoop
								/ 5) {
				...
				} else { // no termination argument and no non-termination argument could be
							// synthesized
					logger.logf(
							WARNING,
	...
	private boolean possiblyNotEqualsNullPointer(final CExpression expr) {
		if (expr instanceof CBinaryExpression) {
			...
			if (binExpr.getOperator() == BinaryOperator.NOT_EQUALS
					...
					&& ((CCastExpression) binExpr.getOperand2())
							.getOperand() instanceof CLiteralExpression) {
				return true;
			}
		}
		return false;
	...
	private void addInvariantsToAggregatedReachedSet(
		...
		rankingRelation.getSupportingInvariants()
				.stream()
	...
	private Set<CVariableDeclaration> getRelevantVariables(Loop pLoop) {
		...
		if (firstLoopHead instanceof FunctionEntryNode) {
			...
			for (CFANode entryNode : FluentIterable.from(pLoop.getLoopNodes())
			...
			if (targetPathIt.hasNext()) {
				...
				if (cfa.getAllNodes().contains(location)
					...
					if (targetPathIt.isPositionWithState() || lastStateInCfa.isPresent()) {
						...
						if (AbstractStates.extractLocation(nextAbstractState)
								.equals(nextLocation)) {
		...
		while (!workList.isEmpty()) {
			...
			Collection<ARGState> parentLoopStates =
					next.getParents()
							.stream()
							.filter(
									p -> extractStateByType(p, TerminationState.class)
											.isPartOfLoop())
	...
	private void resetReachedSet(ReachedSet pReachedSet, CFANode pInitialLocation)
		...
		AbstractState initialState =
				safetyCPA.getInitialState(pInitialLocation, getDefaultPartition());
