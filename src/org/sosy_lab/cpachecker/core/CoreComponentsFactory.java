...
public class CoreComponentsFactory {
	...
	@Option(
		secure = true,
		description = "use a second model checking run (e.g., with CBMC or a different CPAchecker"
	...
	@Option(
		secure = true,
		description = "After an incomplete analysis constructs a residual program which contains all program"
	...
	@Option(
		...
		description = "Use analyses parallely. The resulting reachedset is the one of the first"
	...
	@Option(
		...
		description = "Use termination algorithm to prove (non-)termination. This needs the TerminationCPA as"
	...
	@Option(
		...
		description = "use a proof check algorithm that using pcc.strategy=arg.ARG_CMCStrategy to validate a"
	...
	@Option(
		...
		description = "do analysis and then check if reached set fulfills property specified by"
	...
	@Option(
		...
		description = "use nontermination witness validator to check a violation witness for termination")
	...
	@Option(
		...
		description = "do analysis and then extract pre- and post conditions for custom instruction from"
				+ " analysis result")
	...
	@Option(
		...
		description = "stop the analysis with the result unknown if the program does not satisfies certain"
				+ " restrictions.")
	...
	public Algorithm createAlgorithm(
		...
		if (useUndefinedFunctionCollector) {
			...
			algorithm =
					new UndefinedFunctionCollectorAlgorithm(config, logger, shutdownNotifier, cfa);
			...
			algorithm =
					new NonTerminationWitnessValidator(
							...
							logger,
							shutdownNotifier,
							specification.getSpecificationAutomata());
			...
			algorithm =
					new ConditionalVerifierAlgorithm(
							config,
							logger,
							shutdownNotifier,
							specification,
			...
			algorithm =
					new SelectionAlgorithm(cfa, shutdownNotifier, config, specification, logger);
		} else if (useRestartingAlgorithm) {
			logger.log(Level.INFO, "Using Restarting Algorithm");
			algorithm =
					RestartAlgorithm.create(config, logger, shutdownNotifier, specification, cfa);
		} else if (useCompositionAlgorithm) {
			logger.log(Level.INFO, "Using Composition Algorithm");
			algorithm =
					new CompositionAlgorithm(config, logger, shutdownNotifier, specification, cfa);
			...
			if (constructProgramSlice) {
				...
				algorithm =
						new SlicingAlgorithm(logger, shutdownNotifier, config, cfa, specification);
			}
			...
			if (collectAssumptions) {
				algorithm =
						new AssumptionCollectorAlgorithm(
								algorithm,
								cpa,
								config,
								logger,
			...
			if (splitProgram) {
				algorithm =
						new ProgramSplitAlgorithm(algorithm, cpa, config, logger, shutdownNotifier);
			}
			...
			if (cpa instanceof ARGCPA && forceCexStore) {
				algorithm =
						new CounterexampleStoreAlgorithm(
								algorithm,
								cpa,
								config,
								logger,
			...
			if (useMPV) {
				algorithm =
						new MPVAlgorithm(cpa, config, logger, shutdownNotifier, specification, cfa);
			}

			if (useFaultLocalizationWithCoverage) {
				algorithm =
						new FaultLocalizationWithCoverage(
								algorithm,
								shutdownNotifier,
								logger,
								config);
